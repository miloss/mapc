...
var server = createServer(function (req, res) {
	var method = req.method, handler;
	
	// Create response methods in req object
	...
	// POST
	if (method === "POST") {
		handler = postMap[url.parse(req.url).pathname] || notFound;
		
		// We need to wait until the request's  body is available
		// @see http://phizcode.blogspot.com/2010/03/beyod-static-nodejs-and-post-method.html
		req.addListener('data', function (POST) {
			handler(POST, res);
			
		}).addListener('end', function () {
			// When we get the 'complete' event we're ready to send a response
			// do nothing...
		});
	}
...
fu.staticHandler = function (filename) {
	var filename = this.staticPath + filename;
	var body, headers;
